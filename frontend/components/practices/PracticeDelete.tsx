// components/practices/PracticeDelete.tsx
'use client';

import { useState } from 'react';
import { X, AlertTriangle, Loader2 } from 'lucide-react';

interface PracticeDeleteProps {
  practice: { id: number; name: string };
  onConfirm: (id: number) => Promise<void>;
  onCancel: () => void;
}

export function PracticeDelete({ practice, onConfirm, onCancel }: PracticeDeleteProps) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Supprimer la pratique</h2>
          </div>
          <button onClick={onCancel} className="p-2 hover:bg-gray-100 rounded-lg"><X className="h-5 w-5 text-gray-500" /></button>
        </div>
        <p className="text-gray-600 mb-2">Êtes-vous sûr de vouloir supprimer cette pratique ?</p>
        <p className="text-sm text-gray-500 mb-6">Cette action est irréversible.</p>
        <div className="flex gap-3">
          <button
            onClick={async () => { setLoading(true); await onConfirm(practice.id); setLoading(false); }}
            disabled={loading}
