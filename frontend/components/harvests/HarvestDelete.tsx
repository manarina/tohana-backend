// components/harvests/HarvestDelete.tsx
'use client';

import { useState } from 'react';
import { X, AlertTriangle, Loader2 } from 'lucide-react';

interface HarvestDeleteProps {
  harvest: { id: number; name: string };
  onConfirm: (id: number) => Promise<void>;
  onCancel: () => void;
}

export function HarvestDelete({ harvest, onConfirm, onCancel }: HarvestDeleteProps) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Supprimer la récolte</h2>
          </div>
          <button onClick={onCancel} className="p-2 hover:bg-gray-100 rounded-lg"><X className="h-5 w-5 text-gray-500" /></button>
        </div>
        <p className="text-gray-600 mb-2">Êtes-vous sûr de vouloir supprimer cette récolte ?</p>
        <p className="text-sm text-gray-500 mb-6">Cette action est irréversible.</p>
        <div className="flex gap-3">
          <button
            onClick={async () => { setLoading(true); await onConfirm(harvest.id); setLoading(false); }}
            disabled={loading}
