export default () => ({
  ...
  graphql: {
   ...
   // ✅ GARDER CES CONFIGURATIONS
  security: {
    bcryptRounds: parseInt(process.env.BCRYPT_ROUNDS ?? '', 10) || 10,
    passwordMinLength: parseInt(process.env.PASSWORD_MIN_LENGTH ?? '', 10) || 8,
    maxLoginAttempts: parseInt(process.env.MAX_LOGIN_ATTEMPTS ?? '', 10) || 5,
    blockTimeMinutes: parseInt(process.env.BLOCK_TIME_MINUTES ?? '', 10) || 15,
  },
  ...
  seed: {
    ...
    usersCount: parseInt(process.env.SEED_USERS_COUNT ?? '', 10) || 10,
    farmsCount: parseInt(process.env.SEED_FARMS_COUNT ?? '', 10) || 50,
  },
